...
private:
    ...
    std::string MODEL_PATH = "models/bunny.obj";
    ...
    void createComputePipeline() {
        auto computeShaderCode = readFile("shaders/dancing_particle_comp.spv");
    ...
    void createShaderStorageBuffers() {
        ...
        for (const auto& shape : shapes) {
            for (const auto& index : shape.mesh.indices) {
                ...
                p.position = glm::vec4(attrib.vertices[3 * index.vertex_index + 0] ,
                              attrib.vertices[3 * index.vertex_index + 1] ,
                              attrib.vertices[3 * index.vertex_index + 2], 1.0);
                p.normal = glm::vec4(attrib.normals[3 * index.vertex_index + 0] ,
                                     attrib.normals[3 * index.vertex_index + 1] ,
                                     attrib.normals[3 * index.vertex_index + 2], 0.0);
                p.color = glm::vec4(rndDist(rndEngine), rndDist(rndEngine), rndDist(rndEngine), 1.0);
                p.parameter = glm::vec4(0, rndDist(rndEngine) * 0.02, 4, 0);
                if(glm::dot(p.normal, p.normal) > 0.01)
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.view = glm::lookAt(glm::vec3(4.0f, 4.0f, 4.0f), glm::vec3(-1.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
