...
public:
    ...
    Texture createTextureImageFromFile(std::string path, VkFormat format);
    ...
    void createCommandBuffers() {
        graphicsCommandBuffer.resize(swapChainImages.size());
        ...
        if (vkAllocateCommandBuffers(device, &allocInfo, graphicsCommandBuffer.data()) != VK_SUCCESS) {
    ...
    Buffer createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties) {
        ...
        if (vkCreateBuffer(device, &bufferInfo, nullptr, &resultBuffer.buffer) != VK_SUCCESS) {
            throw std::runtime_error("failed to create buffer!");
        }
        
        VkMemoryRequirements memRequirements;
        vkGetBufferMemoryRequirements(device, resultBuffer.buffer, &memRequirements);
        ...
        if (vkAllocateMemory(device, &allocInfo, nullptr, &resultBuffer.bufferMemory) != VK_SUCCESS) {
            throw std::runtime_error("failed to allocate buffer memory!");
        }
        ...
        resultBuffer.bufferInfo.offset = 0;
        resultBuffer.bufferInfo.range = size;
    ...
    VkPipelineShaderStageCreateInfo loadShader(std::string path, VkShaderStageFlagBits stage){
